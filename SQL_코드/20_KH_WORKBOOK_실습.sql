/* 지금부터 만드는 것은 KH_WORKBOOK 이라는 곳에 만들 테이블 생성 */

CREATE TABLE COMPANY(
COMPANY_ID NUMBER PRIMARY KEY,
COMPANY_NAME VARCHAR(100) NOT NULL,
COMPANY_ADDRESS VARCHAR2(55),
COMPANY_PHONE VARCHAR2(20)
);

-- COMPANY ALTER 테이블명 COMPANY 변경
ALTER TABLE COMPANY RENAME TO COMPANY;
SELECT * FROM COMPANY;

-- COMPANY 시퀀스 생성
-- 시퀀스명 COMPANY_SEQ 시작하는 값 1  증가값 1  NOCACHE NOCYCLE

-- 방법 1
CREATE SEQUENCE COMPANY_SEQ; -- 위에 조건이 모두 기본값이기 때문

-- 방법 2
CREATE SEQUENCE COMPANY_SEQ
START WITH 1 
INCREMENT BY 1
NOCACHE
NOCYCLE
;

-- INSERT COMPANY 회사 추가
INSERT INTO COMPANY 
VALUES (COMPANY_SEQ.NEXTVAL, '햇에제과','서울시 강남구','02-1234-5678');

-- NEXTVAL 이용해서 회사 추가
-- '롯에제가', '서울시 중구', '02-2345-6789'
INSERT INTO COMPANY 
VALUES (COMPANY_SEQ.NEXTVAL, '롯에제가', '서울시 중구', '02-2345-6789');
-- '올이온','서울시 용산구','02-3456-7890'
INSERT INTO COMPANY 
VALUES (COMPANY_SEQ.NEXTVAL, '올이온','서울시 용산구','02-3456-7890');

SELECT * FROM COMPANY;


-- SNACKS 테이블 생성
CREATE TABLE SNACKS(
    SNACK_ID NUMBER(10,0) PRIMARY KEY,
    SNACK_NAME VARCHAR2(100) NOT NULL,
    PRICE NUMBER(10,2) NOT NULL,
    COMPANY_ID NUMBER(10,0),
    CONSTRAINT COMPANY FOREIGN KEY (COMPANY_ID)
    REFERENCES COMPANY(COMPANY_ID)
);

SELECT * FROM SNACKS;

-- SNACKS_SEQ 시퀀스 생성
-- 시퀀스명 SNACKS_SEQ 시작하는 값 1  증가값 1  NOCACHE NOCYCLE 
CREATE SEQUENCE SNACKS_SEQ; -- 방법 1 , 위 조건이 기본 값이기 때문

CREATE SEQUENCE SNACKS_SEQ -- 방법 2 , 위 조건이 기본값이 아니면 여기서 수정
START WITH 1 
INCREMENT BY 1
NOCACHE
NOCYCLE
;

-- SNACKS_SEQ NEXTVAL
INSERT INTO SNACKS
VALUES (snacks_seq.nextval, '오에스','1500','1'); 

INSERT INTO SNACKS
VALUES (snacks_seq.nextval, '칸조','1220','1'); 

INSERT INTO SNACKS
VALUES (snacks_seq.nextval, '배배로','1100','2'); 

INSERT INTO SNACKS
VALUES (snacks_seq.nextval, '조고파이','2000','2'); 

SELECT * FROM SNACKS;

-- 가격이 1500 이상인 과자 조회 -- SNACKS
SELECT *
FROM SNACKS
WHERE PRICE >= 1500;
-- 서울시 강남구 에 위치한 회사 조회 -- COMPANY
SELECT * 
FROM COMPANY
WHERE COMPANY_ADDRESS = '서울시 강남구';
-- 회사 ID 가 1 인 과자 조회 -- SNACKS
SELECT *
FROM SNACKS
WHERE COMPANY_ID = 1;
-- 회사 ID 가 1 이고 그 회사의 과자들 조회 (JOIN)
SELECT C.COMPANY_NAME, S.SNACK_NAME, S.PRICE
FROM COMPANY C
JOIN SNACKS S ON C.COMPANY_ID = S.COMPANY_ID
WHERE C.COMPANY_ID = 1;





























