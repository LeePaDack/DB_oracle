...
-- 1. 각 직원의 급여가 부서 평균 급여보다 높을 직원들의 이름과 급여 조회하기
-- 1) 부서 평균 급여 구하기
...
-- 4. 급여가 300만 이상인 직원 중에서 가장 낮은 급여를 받는 직원의 이름과 급여 조회
SELECT EMP_NAME, SALARY
FROM EMPLOYEE
...
WHERE SALARY >= 3000000;  --> 급여가 300 만원 이상인 행들만 최소값을 계산

외부 쿼리 (메인 쿼리)
SELECT EMP_NAME, SALARY  --> 최종으로 눈으로 볼 컬럼 선택
FROM EMPLOYEE            --> 테이블 선택
WHERE SALARY = (서브 쿼리) --> 급여가 서브쿼리에서 반환된 최소 급여와 같은 행들만 가져오는 것

*/

/* SELECT 를 활용한 서브쿼리 실습 */
-- 1. 부서별로 보너스률이 0.2 이상인 직원의 수 조회
SELECT DEPT_TITLE,
...
---- SELECT 서브쿼리
---- WHERE 서브쿼리
---- 외부 쿼리 SELECT DEPT_TITLE,(서브쿼리) FROM DEPARTMENT WHERE(서브쿼리) >= 5;
