...
sys       : 시스템(system) 단축어
as sysdba : 시스템데이터베이스어드민(SYStem Data Base Admin)
설치할 때 작성한 비밀번호 작성하기
비밀번호는 눈에 보이지 않음
*/
sqlplus sys as sysdba;

-- 기존에는 계정을 생성하기 위해 ALTER 를 이용해서 
-- SCRIPT TRUE 로 변경해줬지만 이제는 하지 않아도 됨
/*
ALTER : 어떤 상황을 변경하는 SQL 명령어
SESSION : 사용자가 로그인해서 명령을 실행하고 연결된 상태
SET : 무엇을 변경할 것인지 선택
"_ORACLE_SCRIPT" : DB 코드 작성을 
TRUE; :          허용하겠다
*/
...
RESOURCE : 테이블, 인덱스, 뷰 등을 생성할 수 있는 권한
        -> T 반에서 특정 행동이나 만들기, 변경, 설정을 할 수 있는 권한
           이 외 여러가지 권한이 있음, 권한을 여러가지 한번에 작성할 때는 , 를 사용해서 작성
...
/* 테이블을 생성하기 
테이블이란?  
    데이터 베이스에서 모든 데이터를 포함하는 공간
    테이블명 : 중점이 되어 중점과 관련된 내용을 설정할 수 있도록 작성
    
생성하는 방법
    CREATE TABLE 태이블명 (컬럼명 데이터타입);
*/
...
VALUES(1, '메가커피', '서울시 강남구 광남동', '02-8888-8888', '영업시간 : 9:00 ~ 18:00');