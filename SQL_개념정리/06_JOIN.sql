...
- 관계형 데이터베이스는 최소한의 데이터를 테이블에 담고 있어
  ...
  두 테이블에서 같은 데이터를 저장하는 컬림이 연결고리가 됨
  
  
등가 JOIN --> 내부조인(INNER JOIN), JOIN USING, ON, 자연조인(NATURAL JOIN)

포괄 JOIN --> 왼쪽외부조인(LEFT OUTER), 오른쪽외부조인(RIGHT OUTER), 전체외부조인(FULL OUTER)
...
-- 서로 다은 테이블 데이터를 조회할 경우 2 가지로 나눠서 조회

-- 직원번호, 직원명, 부서코드, 부서명 을 조회할 때
...
-- JOIN 을 활용해서 EMPLOYEE 테이블과 DEPARTMENT 테이블 한 번에 조회
...
---- WHERE 절에 합치기에 사용할 컬럼명을 명시

-- 1) 연결에 사용할 두 컬럼명이 다른 경우 --> JOIN ON 을 사용
---- ORACLE 에서는 JOIN ON 을 사용하지 않고
----- WHERE 절에 = 만 사용하면 됨

-- ORACLE 용 활용해서 EMPLOYEE 테이블과 DEPARTMENT 테이블 한 번에 조회
SELECT EMP_ID, EMP_NAME, DEPT_CODE, DEPT_ID, DEPT_TITLE
...
JOIN 에 별칭을 설정해서 테이블 표현하기 
JOIN 과 FROM 에서 테이블을 설정할 때
...
FROM EMPLOYEE, DEPARTMENT
WHERE DEPT_CODE = DEPT_ID(+);
-- DEPT_CODE 와 DEPT_ID 같이 일치하지 않아도 
