...
-- 2) 직원들 중에서 총 급여가 3000000 원 이상인  사람들의
-- 사번, 이름, 직급코드, 급여 조회
...
FROM JOB;
--> JOB 테이블에서는 SALARY 가 없기 때문에 에러 발생
...
/*
JOB 테이블에서 JOB_NAME 을 선택

서브 쿼리
EMPLOYEE 테이블에서 전체 직원의 평균 급여를 계산
JOB 테이블의 각 행에 대해 평균 급여가 계산된 서브 쿼리가 실행되기 때문에
각 직급에 대해 동일한 평균 급여값을 반환

위 코드의 문제점 : 각 직급별 개별적인 평균 급여는 반환하지 않음
만약 각 직급별로 평균 급여를 정확하게 계산하고 개별적인 평균 급여도 반환하길 원한다면 
