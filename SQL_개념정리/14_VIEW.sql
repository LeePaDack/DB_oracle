...
1) OR REPLACE
    기존 동일한 이름의 VIEW 가 존재하면 이름 변경
    없으면 새로 생성
    
2) FORCE | NOFORCE
    FORCE : 서브 쿼리에 사용된 테이블이 존재하지 않아도 VIEW 생성
    NOFORCE : (기본값) 서브 쿼리에 사용된 테이블이 존재해야만 VIEW 생성
...
VIEW 는 주로 테이블을 복제하거나 조합해서 테스트용 SELECT 로 많이 사용
...
-- 서울시 강남구에 위치한 회사 조회하는 뷰 COMPANY_IN_GANGNAM
