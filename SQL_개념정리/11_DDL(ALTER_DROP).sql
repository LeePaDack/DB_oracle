...
-- DEPT_COPY 테이블에서 DEPT _TITLE 컬럼에 UNIQUE 제약 조건 추가
...
-- 컬럼 수정 : ALTER TABLE 테이블명 MODIFY 컬럼명 데이터타입;  : 데이터타입을 변경
--           ALTER TABLE 테이블명 MODIFY DEFAULT '값';  : 기본값 변경

---> 데이터 타입 수정할 경우 컬럼에 저장된 크기 미만으로는 변경할 수 없음
----> 데이터 타입을 VARCHAR2(100) -> VARCAHR(10) 하면 데이터 타입이 10 초과되는건 불가

-- 컬럼 삭제 : ALTER TABLE 테이블명 DROP(삭제할컬럼명);
-->          ATLER TABLE 테이블명 DROP COLUMN 삭제할컬럼명;

--> 테이블에는 최소 한 개 이상의 컬럼이 존재해야하기 때문에 모든 컬럼을 삭제 X
...
-- DEPT_COPY 테이블에 LNAME 추가 VARCHAR2(30) 기본값은 한국으로 추가
ALTER TABLE DEPT_COPY ADD (LNAME VARCHAR2(30) DEFAULT '한국');

SELECT * FROM DEPT_COPY;

-- (수정)
-- DEPT_COPY 테이블의 DEPT_ID 컬럼의 데이터 타입을 CHAR(2) -> VARCHATR(3) 으로 변경
ALTER TABLE DEPT_COPY MODIFY DEPT_ID VARCHAR2(3);
--> 주의할 점 : 기존에 저장된 컬럼값이 3 보다 크면 변경 불가
----> 컬럼값을 조절할 때 (큰 값 -> 작은 값) 이미 저장된 데이터값이 변경할 값보다 크지 않은지는 확인하고 조절

-- (삭제)
-- DEPT_COPY 테이블에서 CNAME,LNAME 을 삭제
...
-- 삭제할 테이블 TB1 생성
CREATE TABLE TB1(TB1_PK NUMBER PRIMARY KEY,
                 TB1_COL NUMBER
                 );

-- 삭제할 테이블 TB2 생성
CREATE TABLE TB2(TB2_PK NUMBER PRIMARY KEY,
...
-- 왜래 키에 의해 참조되는 고유 / 기본 키가 존재하기 때문에 삭제할 수 없음
-- 삭제하고 싶은데 어떻게 삭제를 하는가 

-- 해결방법 1 : 자식 테이블(TB2) 삭제 후 부모테이블(TB1) 을 삭제하기
DROP TABLE TB2;
DROP TABLE TB1;
...
DROP TABLE TB2 CASCADE CONSTRAINTS;
SELECT * FROM TB1;
SELECT * FROM TB2;
