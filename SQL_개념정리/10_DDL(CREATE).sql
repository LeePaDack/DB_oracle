...
NULL 값을 넣을 수 없다 설정을 했기 때문에 빈 값(NULL)을 넣을 수 없는 것
*/
...
-- UNIQUE 컬럼 여러개 각 컬럼에다가 중복으로 삽입 가능
-- 테이블, 상품명 / 개수/ 설명(UNIQUE) - NULL / 품절유무(UNIQUE) - NULL 가능 
-- ANSI 국제 표준
...
CREATE TABLE UNIQUE_TABLE(
    ...
    -- (방법 1)
    USER_ID VARCHAR2(20) CONSTRAINT USER_ID_U UNIQUE, -- CONSTRAINT 제약조건명 제약조건 종료
                                                      -- 컬럼 레벨 제약 조건(이름 O)
...
-- CAFE_NAME VARCHAR2(100) NULL 값 못들어가게 설정
...
CREATE TABLE KH_CAFE(
    ...
    CAFE_PHONE VARCHAR(20) CONSTRAINT CAFE_PHONE_U UNIQUE,
    CAFE_ADDRESS VARCHAR(100),
    CONSTRAINT CAFE_ADDRESS_U UNIQUE(CAFE_ADDRESS)
    );
/*
ORA-00955: name is already used by an existing object
OBJECT KH_CAFE 테이블(TABLE = OBJECT)
*/
...
ALTER 테이블명 MODITY(수정할컬럼명 수정할크기);

ALTER KH_CAFE MODIFY(CAFE_PHONE VARCHAR2(20);
--> 정상적으로 번경할 경우 TABLE  KH_CAFE 변경 가능
*/
COMMIT;
...
-------- UNIQUE 와 PRIMARY 의 차이점 --------
-- UNIQUE 는 컬럼에서 NULL 한 개는 허용됨 두 개 이상은 안됨
-- PRIMARY KEY  NULL 값 안되고 중복안됨
...
-- parent key not found 부모키인 USER_GRADE 에 40 이란 값이 없어서 넣을 수 없음

INSERT INTO USER_GRADE VALUES (40, '플레티넘회원');
-- 부모키인 USER_GRADE 에 40 이라는 값이 존재하기 때문에 삽입 가능

INSERT INTO USER_FK VALUES (5,'USER05','PW05',50);
-- parent key not found 부모키인 USER_GRADE 에 50 이란 값이 없어서 넣을 수 없음

INSERT INTO USER_GRADE VALUES (00,'임시회원'); -- 정수에서 맨 앞에 0 사라짐
--> 핸드폰 번호를 기입할 때는 VARCHAR2 를 사용해야하는 이유. 010 이 모두 들어갈 수 있도록 설정
SELECT * FROM USER_GRADE;

-- 만약 USER01 이라는 회원이 일반회원으로 지정이 되어 있는데
-- 직원이 실수로 일반회원을 업애려고 함
--- USER01 이라는 회원의 등급이 사라지기 때문에 마음대로 사라지게 할 수 없도록
---- 처음부터 설정
...
-- 배달 어플 A카페 폐업
-- 배달어플 -> A 카페  카페 마음대로 폐업은 못하지
--- A 카페가 열어놓은 메뉴들을 모두 삭제하기
---- A 카페에 폐업할 때 연결된 메뉴까지 한 번에 삭제
...
(방법2) - 부모키 삭제시 자식키를 NULL 로 변경하는 옵션 : ON DELETE SET NULL
...
-- 앞으로 GENDER 라는 컬럼에는 남이나 여 라는 글자만 들어갈 수 있음
...
/*
ORA-02290: check constraint (KH_T.SYS_C007157) violated
제약 조건으로 남이나 여만 가능하게 했는데 남자라는 조건이 들어와 에러 발생
*/
...
INSERT INTO USER_CHECK 
VALUES (3, 'user03', 'pw03', '강영희', '여자');
-- 에러

-- CHECK 사용 방법

-- 1 번 방법
-- 따로 INDEX 에 기록하지 않고 조건 설정만 할 경우
-- 컬럼명 자료형 CHECK(컬럼명 IN('조건1','조건2'));


-- 2 번 방법
-- 따로 INDEX 에 기록한 다음 조건 설정 할 경우 (한 줄 작성)
-- 컬럼명 자료형 CONSTRAINT 인덱스에기록할이름 CHECK(컬럼명 IN('조건1','조건2'));


-- 3 번 방법
-- 따로 INDEX 에 기록한 다음 조건 설정 할 경우 (여러 줄 작성)
-- 컬럼명 자료형,
