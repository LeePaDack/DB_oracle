...
각 그룹에 월급합인 SUM(SALARY) 를 적용해서
해당부서의 모든 직원의 급여합계를 계산한 것
...
SELECT DEPT_CODE, COUNT(*) AS "보너스 받은 사원 수"
...
/*
EMPLOYEE 테이블에서 부서코드(DEPT_CODE) 직급코드(JOB_CODE) 각 코드 별로 
...
-- HAVING 절 : 그룹 함수로 구해올 구룹에 대한 조건을 설정할 때 사용
--            (그룹에 대한 조건, 어떤 그룹만 조회하겠다)

-- 부서별 평균 급여가 3000000 원 이상인 사원만 조회하고 오름차순 정렬
--> 급여가 300 만원 이상인 지원들만 대상으로 각 부서의 평균 급여 계산
---> 그룹화 전에 필터링이 일어남
...
-- 부서별 평균 급여가 3000000 원 이상인 부서를 조회하고 부서코드 오름차순 정렬
--> 모든 직원들을 대상으로 그룹화한 후, 각 부서별로 평균 급여를 계산
---> HAVING 절을 이용해서 계산된 평균 급여가 300 만원 이상인 부서만 필터링
...
WHERE 저르이 경우 조건에 맞는 값만 가지고 온 후 보여준다면
HAVING 절의 경우 모든 값을 확인하고 조건에 부합하는 그룹만 보여줌
*/

-- 집계 함수(ROLLUP, CUBE)
-- 그룹 별로 산출한 결과 값의 집계를 계산하는 함수
...
ORDER BY DEPT_CODE;
/*
--> EMPLOYEE 테이블에서 DEPT_CODE 와 JOB_CODE 기준으로 SALARY 합계 구함
소계 특정 그룹의 합계
부서코드가 동일한 모든 행의 합계
모든 합계
전체 그룹의 합계
...
D1	  NULL	7820000 - D1 부서 전체의 급여 합계 (소계)
D2	   J4	6520000 - D2 부서의 J4 직급 급여 합계
D2	  NULL	6520000 - D2 부서 전체의 급여 합계 (소계)
D5	   J3	3500000 - D5 부서의 J3 집급 급여 합계
D5	   J5	8460000 - D5 부서의 J5 직급 급여 합계
D5	   J7	3800000
D5	  NULL	15760000 - D5 부서의 전체(J3,J5,J7) 급여 합계 (소계)
D6	   J3	7300000
D6	   J4	2800000
D6	  NULL	10100000
D8	   J6	6986240
...
D5	   J3	    3500000 - D5 부서의 J3 집급 급여 합계
D5	   J5	    8460000 - D5 부서의 J5 직급 급여 합계
D5	   J7	    3800000 - D5 부서의 J7 집급 급여 합계
D5	  NULL	    15760000 - D5 부서의 전체(J3,J5,J7) 급여 합계 (소계)
D6	   J3	    7300000 - D6 부서의 J3 집급 급여 합계
D6	   J4	    2800000 - D6 부서의 J4 집급 급여 합계
D6	  NULL	    10100000 - D6 부서 전체의 급여 합계 (소계)
D8	   J6	    6986240 - D8 부서의 J6 집급 급여 합계
D8	  NULL	    6986240 - D8 부서 전체의 급여 합계 (소계)
D9	    J1	    8000000 - D9 부서의 J1 집급 급여 합계
D9  	J2	    9700000 - D9 부서의 J2 집급 급여 합계
D9	    NULL	17700000 - D9 부서 전체의 급여 합계 (소계)
...
-- 주의할 점 : 집합 연산에 사용되는 SELECT 문은 SELECT 절이 동일해야함

-- UNION 은 OR 과 같은 개념 --> 중복 제거
-- INTERSECT 는 AND 와 같은 개념
-- MINUS 는 차집합 같은 개념
-- UNION ALL 은 OR 결과값에 AND 결과 값이 더해진 값 --> 중복이 제거되지 않은 채로 합쳐진 것
***************/
...
-- 부서코드가 'D5' 이거나 급여갸 300만 초과하는 사원의
-- 이름 부서코드 급여 조회 (중복 포함)
...
-- INTERSECT 를 사용하지 않아도, GROUP BY WHERE 조건으로 구분지을 수 있음
...
-- MINUS 를 사용하지 않아도, GROUP BY WHERE 조건으로 구분지을 수 있음
