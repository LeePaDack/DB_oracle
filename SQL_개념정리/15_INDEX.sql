...
UNIQUE INDEX 가 자동 생성
*/

-- 인덱스 사용 방법 --> WHERE 절에 INDEX 가 지정된 컬럼 언급

SELECT * FROM EMPLOYEE; -- 인덱스를 사용하지 않고 단순히 조회

SELECT * FROM EMPLOYEE
WHERE EMP_ID != 0; -- EMP_ID 라는 INDEX 를 사용한다는 의미
--> EMP_ID 가 PRIMARY KEY 로 정렬하는 숫자값으로 되어있기 때문에 가능

--> INDEX 는 숫자로 무언가를 찾기 위한 값

--> 왜 실행시 없다고 하나? 계정이 KH_WORKBOOK 이기 때문 --> KH_T 계정으로 변경
