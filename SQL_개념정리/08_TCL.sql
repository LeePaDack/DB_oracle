...
Ex) 
    ...
    INSERT INTO 10 번 값 추가 ---> 1 개 트랜잭션에 10 개 추가 -- ROLLBACK ---> DB 반영 X
    
    1) COMMIT : 메모리 트랜잭션에 임시 저장된 데이터를 DB 에 완전히 추가
    
    2) ROLLBACK : 메모리 트랜잭션에 임시 저장된 데이터를 삭제하고 
                  마지막 COMMIT 상태로 돌아감
    
    3) SAVEPOINT : 메모리 트랜잭션에 저장 지점을 정의 
                   ROLLBACK 을 수행할 경우 전체 작업을 삭제하는 것이 아닌
...
ROLLBACK; -- 완벽하게 저장이 된 상태에서 돌아가기를 진행하면
-- 커밋이 완료된 시점으로 돌아감
...
-- DEPT_ID D8 인 행을 삭제, 삭제한 내역 되돌리기
DELETE FROM DEPARTMENT3
WHERE DEPT_ID = 'D8';

ROLLBACK; -- 마지막 커밋 이후의 것들을 없앰
...
SELECT * FROM DEPARTMENT3; -- 실수로 전체 삭제

ROLLBACK;
-- AAA 까지 돌아가고싶다
ROLLBACK TO "AAA"; --- AAA 시점으로 저장된 DB 내역이 돌아옴
